...
(defun test-exploration ()
  (let ((c (make-instance 'quad-tree
    ...
    (insert-item c (make-node-for-container c (cons 3.1 3.1)))
    (insert-item c (make-node-for-container c (cons -1 1)))
    (insert-item c (make-node-for-container c (cons 1 1)))
    (insert-item c (make-node-for-container c (cons 1 -1)))
    (insert-item c (make-node-for-container c (cons -1 -1)))
    (insert-item c (make-node-for-container c (cons 0 1)))
    (insert-item c (make-node-for-container c (cons 2 2)))
    (insert-item c (make-node-for-container c (cons 3 3)))
    (insert-item c (make-node-for-container c (cons 4 4)))
    (insert-item c (make-node-for-container c (cons -2 1)))
    (unless (= (size c) 11)
      (cerror "continue" "error: got ~a expected n" (size c)))
